...
def index():
    ...
    values = 'mercedes'
    cur.execute("SELECT * FROM products WHERE category=%s ORDER BY RAND() LIMIT 4", (values,))
    mercedes = cur.fetchall()
    values = 'ferrari'
    cur.execute("SELECT * FROM products WHERE category=%s ORDER BY RAND() LIMIT 4", (values,))
    ferrari = cur.fetchall()
    # Close Connection
    cur.close()
    return render_template('home.html', audi=audi, bmw=bmw, mercedes=mercedes, ferrari=ferrari, form=form)
...
def audi():
    ...
    if request.method == 'POST' and form.validate():
        ...
        return render_template('audi.html', audi=products, form=form)
    if 'view' in request.args:
        ...
        if 'uid' in session:
            ...
            if result:
                ...
                mysql.connection.commit()
        return render_template('view_product.html', x=x, audi=product)
        ...
        product = curso.fetchall()
        x = content_based_filtering(product_id)
        return render_template('order_product.html', x=x, audi=product, form=form)
    return render_template('audi.html', audi=products, form=form)
...
def bmw():
    ...
    if request.method == 'POST' and form.validate():
        ...
        flash('Order successful', 'success')
        return render_template('bmw.html', bmw=products, form=form)
    if 'view' in request.args:
        q = request.args['view']
        product_id = q
        x = content_based_filtering(product_id)
        ...
        return render_template('view_product.html', x=x, audi=products)
    elif 'order' in request.args:
        product_id = request.args['order']
        curso = mysql.connection.cursor()
        curso.execute("SELECT * FROM products WHERE id=%s", (product_id,))
...
def mercedes():
    ...
    if request.method == 'POST' and form.validate():
        ...
        flash('Order successful', 'success')
        return render_template('mercedes.html', mercedes=products, form=form)
    if 'view' in request.args:
        q = request.args['view']
        product_id = q
        x = content_based_filtering(product_id)
...
def ferrari():
    ...
    if request.method == 'POST' and form.validate():
        ...
        flash('Order successful', 'success')
        return render_template('ferrari.html', ferrari=products, form=form)
    if 'view' in request.args:
        q = request.args['view']
        product_id = q
        x = content_based_filtering(product_id)
...
def admin_login():
    if request.method == 'POST':
        ...
        if username == 'admin@gmail.com' and password_candidate == 'admin':
            # passed
            session['admin_logged_in'] = True
            session['admin_uid'] = 1  # Assuming the admin ID is 1
        ...
        if name and price and description and available and category and item and code and file:
            ...
            if picture.lower().endswith(('.png', '.jpg', '.jpeg')):
                ...
                if save_photo:
                    ...
                    if category == 'audi':
                        level = request.form.getlist('audi')
                        for lev in level:
                            yes = 'yes'
                        ...
                        level = request.form.getlist('bmw')
                        for lev in level:
                            yes = 'yes'
                            query = 'UPDATE product_level SET {field}=%s WHERE product_id=%s'.format(field=lev)
                            curs.execute(query, (yes, product_id))
                            # Commit cursor
                            mysql.connection.commit()
                    elif category == 'mercedes':
                        level = request.form.getlist('mercedes')
                        for lev in level:
                            yes = 'yes'
                            query = 'UPDATE product_level SET {field}=%s WHERE product_id=%s'.format(field=lev)
                            curs.execute(query, (yes, product_id))
                        ...
                        level = request.form.getlist('ferrari')
                        for lev in level:
                            yes = 'yes'
                            query = 'UPDATE product_level SET {field}=%s WHERE product_id=%s'.format(field=lev)
                            curs.execute(query, (yes, product_id))
